...
namespace KPI.Code
{
    public class ExcelHelper
    {
        ...
        {
            DataTable dt_Column = excelConfig.ColumnEntity.ToJson().ToTable();
            for (int i = dtSource.Columns.Count - 1; i >= 0; i--)
            {
                DataColumn column = dtSource.Columns[i];
                DataRow[] daRow = dt_Column.Select(string.Format("column = '{0}'", column.ColumnName));
                bool flag = daRow.Count() > 0;
                if (!flag)
                {
                ...
                {
                    dtSource.Columns[i].ColumnName = daRow[0]["ExcelColumn"].ToString();
                }
            }
        ...
        public static MemoryStream ExportListByTempale(List<TemplateMode> list, string templdateName)
        {
            try
            {
                ...
                string templatePath = HttpContext.Current.Server.MapPath("/") + "/Resource/Moban/";
