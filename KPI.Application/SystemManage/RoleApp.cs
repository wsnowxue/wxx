...
namespace KPI.Application.SystemManage
{
    public class RoleApp
    {
        ...
        public List<RoleEntity> GetDutyList(string keyword = "")
        {
            var expression = ExtLinq.True<RoleEntity>();
            if (!string.IsNullOrEmpty(keyword))
            {
        ...
        //获取除了中安总部外的所有分公司
        public RoleEntity GetByFullName(string FullName,int? CategoryId)
        {
            var expression = ExtLinq.True<RoleEntity>();
            expression = expression.And(t => t.F_FullName.Equals(FullName));
            if (CategoryId != null) {
                expression = expression.And(t => t.F_Category==CategoryId);
            }
