...
namespace KPI.Application.SystemManage
{
    public class RoleAuthorizeApp
    {
        ...
        public bool ActionValidate(string roleId, string moduleId, string action)
        {
            ...
            var cachedata = Cache.GetCache<List<AuthorizeActionModel>>("authorizeurldata_" + roleId);
            if (cachedata == null)
            {
                var moduledata = moduleApp.GetList();
                var buttondata = moduleButtonApp.GetList();
                ...
                Cache.WriteCache(authorizeurldata, "authorizeurldata_" + roleId, DateTime.Now.AddMinutes(5));
            }
            else
            {
